...
class GameMenuControl {
    ...
    public void takeTurn() {
        ...
        if (!this.game.status.equals(Game.NEW_GAME)  && 
            ...
            BattleshipsError.displayError("You must start a new game first.");
            return;
        }
    ...
    private Point getComputersSelection() {
        ...
        if (coordinate == null) { // no winning location found for other player
            ...
            if (coordinate == null) {
                Object displayError = BattleshipsError.displayError("No empty locations found on the board");
                return null;
            }
        }
    ...
    private boolean isTie() {
        ...
        battleship1.Location[][] locations = this.board.boardLocations;
        
        for (Location[] rowLocations : locations) {
            for (Location location : rowLocations) {
                if (location.player == null) {
                    // location not taken yet?
                    return false;
                }
            }
        }
    ...
    private boolean isWinner() {
        ...
        for (int row = 0; row < locations.length; row++) {
            ...
            battleship1.Location[] rowLocations = locations[row];
            
            // for every column in the row
            for (int col = 0; col < rowLocations.length; col++) {
                
                // three of the same players Ships in a row?
                if (fiveHits(row, col, locations)) { 
    ...
    /* 
     * Are there three of the same Ships in a row
     */
    private boolean fiveHits(int row, int col, battleship1.Location[][] boardLocations) {
    ...
    private Point findWinningLocation(Players player) {
        ...
        for (int row = 0; row < locations.length; row++) {
            battleship1.Location[] rowLocations = locations[row];
            for (int col = 0; col < rowLocations.length; col++) {
                battleship1.Location location = rowLocations[col];
                ...
                // search for three adjacent horizontally
                if ((locations[row][col + 1].player == player
